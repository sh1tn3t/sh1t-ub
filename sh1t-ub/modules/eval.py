...
class EvaluatorMod(loader.Module):
    ...
    async def exec_cmd(self, app: Client, message: types.Message):
        """Выполнить python-код"""
        return await self.execute(app, message)

    async def eval_cmd(self, app: Client, message: types.Message):
    ...
    async def execute(self, app: Client, message: types.Message, return_it: bool = False):
        ...
        try:
            ...
            return await message.answer(
                f"<b>[{'eval' if return_it else 'exec'}] Не удалось выполнить выражение:</b>\n"
                f"<code>{args}</code>\n\n"
                f"<b>Ошибка:</b>\n"
        ...
        if return_it:
            ...
            await message.answer(f"{outputs[0]}</code>")
